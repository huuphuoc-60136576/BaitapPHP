...
<body>
    <form action="" method="get">
        <table bgcolor="#eeeeee" align="center" width="70%" border="1"
            ...
            <tr>
                <td align="center">
                    <font color="blue"><h3>TÌM KIẾM THÔNG TIN SỮA</h3></font>
                </td>
            </tr>
            <tr>
                <td align="center">
                    Tên sữa: <input type="text" name="tensua" size="30"
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'GET') {
            if (empty($_GET['tensua'])) echo "<p align='center'>Vui lòng nhập tên sản phẩm</p>";
                ...
                $query="SELECT sua.*, Ten_hang_sua from Sua,hang_sua
                        WHERE sua.ma_hang_sua = hang_sua.ma_hang_sua AND Ten_sua like '%$tensua%'";
                $result = mysqli_query($dbc, $query);
                if (mysqli_num_rows($result)<>0) {
                    $row = mysqli_num_rows($result);
                    echo "<div align='center'><b>Có $row sản phẩm được tìm thấy.</b></div>";
